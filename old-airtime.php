...
<?php vp_kyc_update();?>
    ...
    <div class="mb-2">
		...
		<label for="network_type" class="form-label">Airtime Type</label><br>
			<select id="airtimechoice" name="airtimechoice" class="form-select form-select-sm" aria-label=".form-select-sm example">
		...
		<div class="mb-2">
         <label for="network" class="form-label">Network</label>
            <select class="form-select form-select-sm airtime-network" aria-label="form-select-sm example" id="net" name="network">
                <option value="none" selected>---Select---</option>
				
				<?php
				if(vp_option_array($option_array,"vtucontrol") == "checked"){
				?>
				<div class="vtu_airtime">
                <option value="<?php echo vp_option_array($option_array,'airtimemtn');?>" class="vtuairtime">MTN</option>
				<option value="<?php echo vp_option_array($option_array,'airtimeglo');?>" class="vtuairtime">GLO</option>
				<option value="<?php echo vp_option_array($option_array,'airtimeairtel');?>" class="vtuairtime">AIRTEL</option>
				<option value="<?php echo vp_option_array($option_array,'airtime9mobile');?>" class="vtuairtime">9MOBILE</option>
				</div>
				<?php
				}
				?>
				<?php
				if(vp_option_array($option_array,"sharecontrol") == "checked"){
				?>
				<div class="share_airtime">
                <option value="<?php echo vp_option_array($option_array,'sairtimemtn');?>" class="shareairtime">MTN</option>
				<option value="<?php echo vp_option_array($option_array,'sairtimeglo');?>" class="shareairtime">GLO</option>
				<option value="<?php echo vp_option_array($option_array,'sairtimeairtel');?>" class="shareairtime">AIRTEL</option>
				<option value="<?php echo vp_option_array($option_array,'sairtime9mobile');?>" class="shareairtime">9MOBILE</option>
				</div>
				<?php
				}
				?>
				<?php
				if(vp_option_array($option_array,"awufcontrol") == "checked"){
				?>
				<div class="awuf_airtime">
                <option value="<?php echo vp_option_array($option_array,'wairtimemtn');?>" class="awufairtime">MTN</option>
				<option value="<?php echo vp_option_array($option_array,'wairtimeglo');?>" class="awufairtime">GLO</option>
				<option value="<?php echo vp_option_array($option_array,'wairtimeairtel');?>" class="awufairtime">AIRTEL</option>
				<option value="<?php echo vp_option_array($option_array,'wairtime9mobile');?>" class="awufairtime">9MOBILE</option>
				</div>
				<?php
				}
				?>
...
if(current_user_can("administrator")){
	?>
                <button type="button" class="btn btn-secondary view-url">View Url</button>
<?php 
}
?>
			<?php
if(vp_option_array($option_array,"vpdebug") != "yes" || current_user_can("administrator")){
	?>
                <button type="button" class="w-full p-2 text-xs font-bold text-white uppercase bg-indigo-600 rounded shadow purchase-airtime btn" data-bs-toggle="modal" data-bs-target="#exampleModal" data-bs-whatever="@getbootstrap">Purchase</button>
<?php
}
else{
    ?>
            ...
            <div class="modal-dialog">
              <div class="modal-content">
                ...
                <div class="modal-footer">
                  <button type="button" class="p-2 text-xs font-bold text-dark uppercase bg-gray-600 rounded shadow airtime-proceed-cancled btn-danger" data-bs-dismiss="modal">Cancel</button>
                  <button type="button" name="wallet" id="wallet" class=" p-2 text-xs font-bold text-white uppercase bg-indigo-600 rounded shadow airtime-proceed btn-success" form="cfor">Proceed</button>
                </div>
              </div>
            </div>
...
//CHECK FOR AIRTIMECHOICE VAL BY DEFAULT
//DEV NOTE: PHP parse here is highly prohibited \/\/\/{3 strings}
//# Hask[MD5-MODE 6] runtime x645-221, BM\Tf119
var airtimechoice = jQuery("select#airtimechoice").val();

//alert(airtimechoice);

if(airtimechoice == "vtu"){
jQuery("option.shareairtime").hide();
jQuery("option.awufairtime").hide();
jQuery("option.vtuairtime").show();
}
if(airtimechoice == "share"){
jQuery("option.shareairtime").show();
jQuery("option.awufairtime").hide();
jQuery("option.vtuairtime").hide();
}
if(airtimechoice == "awuf"){
jQuery("option.shareairtime").hide();
jQuery("option.awufairtime").show();
jQuery("option.vtuairtime").hide();
}
if(airtimechoice == "none"){
jQuery("option.shareairtime").hide();
jQuery("option.awufairtime").hide();
jQuery("option.vtuairtime").hide();
}
...
jQuery("select#airtimechoice").on("change",function(){
var airtimechoice = jQuery("select#airtimechoice").val();
